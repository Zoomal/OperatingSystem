...
struct thread* tid_to_thread(tid_t tid)
{
	...
	while(e)
	{
		...
		if(e==list_end(&ready_list)) { return thr; }
		if(thr->tid == tid) 
		{
			return thr;
		}
               ...
               thread_func *function, void *aux) 
{
  ...
  if (t == NULL)
    return TID_ERROR;
  /* Initialize thread. */
//  printf("in thread_create(), thread is created\n");
...
#ifdef USERPROG
  if(thread_current()->exit_status <0) thread_current()->exit_status=-1;
