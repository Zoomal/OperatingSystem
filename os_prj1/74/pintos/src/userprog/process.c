...
process_execute (const char *file_name) 
{
  ...
  tid = thread_create (tmp_argv[0], PRI_DEFAULT, start_process, fn_copy);
  if (tid == TID_ERROR)
...
process_wait (tid_t child_tid UNUSED) 
{
	...
	if(child->tid != child_tid) return -1; // parameter child_tid is not a direct child of the calling process
	if(!(t!=NULL)&&(t->magic==0xcd6abf4b)) return -1; // if current thread isn't proper
	
	int exit_status;
	intr_disable(); //make parent's intr disable
	thread_block(); //sleep current thread -> it makes it's status THREAD_BLOCKED
...
static bool load_segment (struct file *file, off_t ofs, uint8_t *upage,
                          ...
                          bool writable);
//static bool make_stack(void **esp, char*** argv, uint32_t argc);
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  //char* buf;
  //buf=(char*)malloc(sizeof(char)*strlen(file_name));
  char *token;
  char tmp[30][10]={};
  char *ptr[2];
  int k=0;
  ...
  //strlcpy(buf,file_name,strlen(file_name));
  token = strtok_r(file_name," ",&ptr[0]);
  //printf("token : %s\n",token);
  //printf("ptr[0] : %s\n",ptr[0]);
  ...
  int len;
  uint32_t adrs_arr[20];
  char temp_char[20];

  if(is_user_vaddr(esp-4)) exit(-1);

  /* PUT DATA OF ARGUMENTS */
  for(p=argc-1;p>=0;p--)
  {
	  len=strlen(argv[p])+1;
	  *esp -= len;
	  memcpy(*esp,&(argv[p]),len-1);
	  adrs_arr[p]=*esp;
	  if(len%4!=0) word_align+=((((int)(len/4)+1)*4)-len);
  }

  /* put word align */
  word_align %= 4;
  *esp -= word_align;
  memset(*esp,(uint8_t)0,word_align);
  ...
  *esp -= 4;
  ...
  {
	  *esp -= 4;
	  memcpy(*esp,&(adrs_arr[p]),4);
	  if (p==0) {argv_adrs = *esp;}
  }
  
  /* put argv */
  *esp -= 4;
  memcpy(*esp,&argv_adrs,4);
